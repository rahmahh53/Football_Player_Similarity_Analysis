...
select p.player_id, p.player_name, pp.primary_position_id, pp.position_event_count, case when pp.primary_position_id = 1 then 'Goalkeeper' when pp.primary_position_id between 2 and 8 then 'Defender' when pp.primary_position_id in (9, 10, 11, 12, 13, 14, 15, 16, 18, 19, 20) then 'Midfielder' when pp.primary_position_id in (17, 21, 22, 23, 24, 25) then 'Forward' else 'Unknown' end as role_group, coalesce(pf.total_passes, 0) as total_passes, coalesce(pf.completed_passes, 0) as completed_passes, coalesce(pf.progressive_passes, 0) as progressive_passes, round(100.0 * coalesce(pf.completed_passes, 0)/nullif(pf.total_passes, 0), 2) as pass_completion_rate,
...
left join shot_features as sf on p.player_id = sf.player_id left join dribble_features as df on p.player_id = df.player_id left join carry_features as cf on p.player_id = cf.player_id left join appearance_features as af on p.player_id = af.player_id left join primary_positions as pp on p.player_id = pp.player_id where coalesce(af.matches_played, 0) >= 5;
